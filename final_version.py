...
game_over_text_rect.center = (width//2, height//2 - 20)
...
take_bomb_sound = pygame.mixer.Sound("media/bomb.wav")
take_bomb_sound.set_volume(2)
...
bomb_image = pygame.image.load("img/Bomb-icon.png")
bomb_image_rect = bomb_image.get_rect()
bomb_image_rect.x = width + bomb_behind_border
bomb_image_rect.y = random.randint(110, height - 48)
...
while lets_continue:
    ...
    # Funkce pro střídání mincí a bomb po skóre 10
    def spawn_new_object():
        global current_object
        if score > 10:
            current_object = random.choice(["coin", "bomb"])  # Náhodné střídání
        else:
            current_object = "coin"  # Do skóre 10 vždy mince

    # Logika pohybu a kolize objektů
    if current_object == "coin":
        if coin_image_rect.x < 0:  # Mince je mimo obrazovku
            player_lives -= 1
            spawn_new_object()  # Vyber nový objekt
            coin_image_rect.x = width + coin_behind_border
            coin_image_rect.y = random.randint(110, height - 85)
        ...
        if player_image_rect.colliderect(coin_image_rect):
            ...
            spawn_new_object()  # Vyber nový objekt
            coin_image_rect.x = width + coin_behind_border
            coin_image_rect.top = random.randint(60, height - 85)
            take_coin_sound.play()

    elif current_object == "bomb":
        if bomb_image_rect.x < 0:  # Bomba je mimo obrazovku
            spawn_new_object()  # Vyber nový objekt
            bomb_image_rect.x = width + bomb_behind_border
            bomb_image_rect.top = random.randint(110, height - 85)

        # Pohyb bomby doleva
        bomb_image_rect.x -= coin_curent_speed

        # Kolize s hráčem
        if player_image_rect.colliderect(bomb_image_rect):
            player_lives -= 1
            spawn_new_object()  # Vyber nový objekt
            bomb_image_rect.x = width + bomb_behind_border
            bomb_image_rect.top = random.randint(60, height - 85)
            take_bomb_sound.play()
    ...
    # Tvary
    pygame.draw.line(screen, white, (0, 80), (width, 80), 5)
    ...
    if player_lives == 0:
        ...
        your_best_score_text_rect.center = (width//2, height//2 + 140)
        your_act_score_text = mario_font_g.render(f"Tvoje Skore Bylo: {score}", True, blue)
        your_act_score_text_rect = your_act_score_text.get_rect()
        your_act_score_text_rect.center = (width//2, height//2 + 230)
        screen.blit(your_best_score_text, your_best_score_text_rect)
        screen.blit(your_act_score_text, your_act_score_text_rect)
        pygame.display.update()
